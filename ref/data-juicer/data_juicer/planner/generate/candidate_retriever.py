...
from data_juicer.planner.generate.candidate_ranker import _MUST_INCLUDE
from data_juicer.planner.generate.embedding.backend import EmbeddingBackend
from data_juicer.planner.generate.vector_index import OperatorVectorIndex
from data_juicer.tools.op_search import OPRecord, OPSearcher

logger = logging.getLogger(__name__)


def _detect_modalities(intent: str, dataset_hint: str) -> Set[str]:
    """Detect modalities from user intent (reuse from candidate_filter)."""
    from data_juicer.planner.generate.candidate_filter import detect_modalities
    return detect_modalities(intent, dataset_hint)


def _filter_ops_by_modality(records: List[OPRecord], modalities: Set[str]) -> List[OPRecord]:
    """Filter operators by modality (reuse from candidate_filter)."""
    from data_juicer.planner.generate.candidate_filter import filter_ops_by_modality
    return filter_ops_by_modality(records, modalities)
...
class VectorRetriever:
    ...
    def __init__(
        ...
        self._all_records: List[OPRecord] = []
    ...
    def retrieve(
        ...
        if use_modality_filter:
            modalities = _detect_modalities(intent, dataset_hint)
            if modalities:
                filtered_records = _filter_ops_by_modality(self._all_records, modalities)
                if filtered_records:
